...
const products = [
  {
    ...
    category: "protein",
    price: 85000,
    imageFile: "iso-xp-whey-protein-isolate.webp",
...
// Catalog Modal Elements (Professional product view)
...
const openCatalogTriggers = document.querySelectorAll('.open-catalog');
...
function formatPrice(price) {
    // Return numbers only; currency label is handled in CSS to avoid duplication.
...
function renderProducts(productsToRender) {
    ...
    productsToRender.forEach(product => {
        ...
        productCard.className = 'product-card scroll-reveal visible';
        ...
        // IMPORTANT: Keep the "All products" view identical to other categories.
        // We stack name + price under the image to prevent any layout bugs on small screens.
        productCard.innerHTML = `
            ...
            <div class="product-info">
                ...
                <div class="product-meta">
                    <span class="meta-pill"><i class="fas fa-layer-group"></i> ${getArabicCategory(product.category)}</span>
                    ${product.badge ? `<span class="meta-pill"><i class="fas fa-tag"></i> ${product.badge}</span>` : ''}
                </div>
...
function openCatalog(filter = 'all') {
    ...
    // Close other overlays for cleanliness
    cartSidebar?.classList.remove('active');
    mobileMenu?.classList.remove('active');

    currentCatalogFilter = filter || 'all';
    currentCatalogQuery = (catalogSearchInput?.value || '').trim();
    currentCatalogSort = catalogSortSelect?.value || 'default';

    catalogModal.classList.add('active');
    document.body.classList.add('modal-open');
    catalogModal.setAttribute('aria-hidden', 'false');

    syncActiveFilterButtons(currentCatalogFilter);
    applyCatalogFilters();

    // Focus search for fast UX
...
function syncActiveFilterButtons(filter) {
...
function applyCatalogFilters() {
    ...
    // Category
    if (currentCatalogFilter && currentCatalogFilter !== 'all') {
        filtered = filtered.filter(p => p.category === currentCatalogFilter);
    }

    // Search
    const q = (currentCatalogQuery || '').toLowerCase();
    if (q) {
...
function setupEventListeners() {
    ...
    // Open catalog triggers (nav/buttons)
    openCatalogTriggers.forEach(trigger => {
        trigger.addEventListener('click', function(e) {
            // allow button clicks without jumping
    ...
    catalogModal?.querySelector('[data-close="true"]')?.addEventListener('click', closeCatalog);
    ...
    catalogSearchInput?.addEventListener('input', function() {
        currentCatalogQuery = this.value.trim();
        applyCatalogFilters();
    });

    catalogSortSelect?.addEventListener('change', function() {
        currentCatalogSort = this.value;
        applyCatalogFilters();
    });
    ...
    // Category cards -> open catalog with the selected category
